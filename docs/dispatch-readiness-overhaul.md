...
correctly _given the framing_: fork → repro → fix → submission_judge passed
...
**The keycloak shape is silent.** It sits in the 23-issue _pass_ column —
...
This doc tracks the multi-phase overhaul to detect _upstream receptiveness_
...
- `open_stale` — open with no maintainer engagement; measured at 30d and 90d
  _checkpoints on the same trajectory_, not as disjoint buckets. 90d is the
...
closed-unmerged categories. Internal gates passing is a _necessary_ condition
for shipping but not a _sufficient_ condition for success. Without
...
Codes evolve; the point is making override reasons _queryable_ from day one
so the calibration corpus is rich enough to tune the actionability rubric
against.

**(c) Per-gate decision telemetry.** _(Non-blocking — nice-to-have for
...
- [ ] _(non-blocking)_ Uniform gate-decision evidence: every gate writes
      `<batch>/<issue>/gate_decisions/<gate_name>.json` with
      `{verdict, score?, reasoning, evidence, at}`.
- [ ] _(non-blocking)_ Backfill the existing batches' gate decisions into
...
Tier 0 is _scraper-side budget protection that prevents the scraper from
spending API on issues we'd reject downstream anyway_ — not a new
filtering layer.
...
repo _adopted_ the sub-issues feature. Older or feature-non-adopting repos
can be epic-shaped with zero sub-issues. Mitigation: treat sub-issue count
as a _strong_ signal where present, and treat `epic` / `tracking` /
`umbrella` labels and a comment-mention pattern (`> 5 distinct
maintainers debating scope`) as _parallel_ signals. Not "paired" — these
are independent OR'd indicators of epic shape.

### Cost framing

Per-repo request count stays comparable to today (≈170 req/repo).
**Savings aren't the point.** What changes is _data quality on the
population that matters_: comments cover every dispatch candidate (not
...
- [ ] Persist Tier-0 reject reasons to a new KV namespace
      `recon:{slug}:rejects` for operator audit. _(New shape — no precedent
...
deterministic _cost-control circuit-breaker_ score so vibedispatch can
...
| **Readiness** (dispatchReadinessScore) | Can we dispatch on it _right now_? | Fast — volatile       | Vibedispatch's pre-dispatch gate |
...
_against_ keycloak then validating _against_ keycloak is circular. The
